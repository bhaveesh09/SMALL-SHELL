...
title "CS344 SU22 smallsh Grading Script"
...
header 5 "simple command execution"
if cmp -s <(/bin/whoami) <(smallsh /bin/whoami); then
  pass "executed successfully"
  POINTS=$((POINTS + 5))
else
  fail "whoami does not match expected"
  diff -uN <(/bin/whoami) <(smallsh /bin/whoami) | sed 's/^/    /' | tail -n +4
fi

header 10 "command execution with arg"
if cmp -s <(ls -i -sh -U -1 --color=never) <(smallsh 'ls -i -sh -U -1 --color=never'); then
  pass "executed successfully"
  POINTS=$((POINTS + 10))
else
  fail "listing does not match expected"
  diff -uN <(ls -i -sh -U -1 --color=never) <(smallsh 'ls -i -sh -U -1 --color=never') | sed 's/^/    /' | tail -n +4
fi
...
header 5 "input and output redirection"
...
header 10 "input and output redirection with args"
smallsh "wc -L -w -l -c > junk3 < junk"
if cmp -s <(wc -L -w -l -c <junk) junk3; then
  pass "file exists and content is correct"
  POINTS=$((POINTS + 10))
else
  fail "contents do not match"
  ...
  echo "    diff:"
  diff -uN <(wc -L -w -l -c <junk) junk3 | sed 's/^/    /' | tail -n +4
fi
...
header 5 "testing cd without arguments"
OUTPUT=$(smallsh 'cd' 2>&1 > /dev/null)
echo $OUTPUT | grep -qP '.+' # /.+/ regex matches any output
if [ $? -eq 0 ]; then
  pass "cd successfully detected invalid parameter count"
  POINTS=$((POINTS + 5))
else
  fail "cd failed to detect error"
  info "cd should print error to stderr if no argument is provided"
fi

header 5 "cd to subdir"
...
echo -e 'echo $PD > junkpid\nexit' | $BIN_DIR/smallsh >/dev/null &
wait $!
if [ $! = $(cat junkpid) ]; then
  pass "pid var correctly replaced"
  POINTS=$((POINTS + 5))
else
  [ $(cat junkpid) = '$PD' ] && fail "pid var was not replaced" || fail "replacement does not match correct pid"
  info "was $(cat junkpid), expected $!"
fi
...
title "FINAL SCORE: ${BLUE}${POINTS}${WHITE} / 185"
